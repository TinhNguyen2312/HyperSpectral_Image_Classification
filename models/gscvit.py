...
class ChannelAdjustmentLayer2(nn.Module):
    ...
    def forward(self, x):
        ...
        if C < self.target_channels:
            ...
            x = torch.cat(
                [
                    x[:, : channels_to_expand_per_side + 1, :, :].flip(dims=(1,)),
                    x,
                    x[:, -channels_to_expand_per_side:, :, :].flip(dims=(1,)),
                ],
                dim=1,
            )
...
class GSSA(nn.Module):
    def __init__(self, dim, heads=8, dim_head=16, dropout=0.0, group_spatial_size=3):
        super().__init__()
        self.heads = heads
        ...
        self.group_attend = nn.Sequential(nn.Softmax(dim=-1), nn.Dropout(dropout))

        self.to_out = nn.Sequential(nn.Conv2d(inner_dim, dim, 1), nn.Dropout(dropout))

    def forward(self, x):
        batch, height, width, heads, gss = (
            x.shape[0],
            *x.shape[-2:],
            self.heads,
            self.group_spatial_size,
        )
        ...
        if num_groups == 1:
            fmap = rearrange(
                grouped_fmaps,
                "(b x y) h (g1 g2) d -> b (h d) (x g1) (y g2)",
                x=height // gss,
                y=width // gss,
                g=gss,
                g2=gss,
        ...
        group_tokens = rearrange(
            group_tokens, "(b x y) h d -> b h (x y) d", x=height // gss, y=width // gss
        )

        grouped_fmaps = rearrange(
            grouped_fmaps,
            "(b x y) h n d -> b h (x y) n d",
            x=height // gss,
            y=width // gss,
        )
        ...
        aggregated_grouped_fmap = einsum(
            "b h i j, b h j w d -> b h i w d", w_attn, grouped_fmaps
        )

        fmap = rearrange(
            aggregated_grouped_fmap,
            "b h (x y) (g1 g2) d -> b (h d) (x g1) (y g2)",
            x=height // gss,
            y=width // gss,
            g1=gss,
            g2=gss,
...
class GSCViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        hyperparams_per_stage = list(
            map(partial(cast_tuple, length=num_stages), hyperparams_per_stage)
        )
        assert all(
            tuple(map(lambda arr: len(arr) == num_stages, hyperparams_per_stage))
        )
                ...
                nn.ModuleList(
                    [
                        ...
                        Transformer(
                            ...
                            depth=layer_depth,
                            heads=layer_heads,
                            groupsize=group_spatial_size[ind],
                            dropout=dropout,
