...
def load_mat_hsi(dataset_name, dataset_dir):
    ...
    available_sets = [
        ...
        "ksc",
        "ip",
        "ipnpy",  # Indian Pines loaded from .npy
        "hus",
        "MUUFL",
        "Trento",
        "botswana",
    ...
    assert dataset_name in available_sets, (
        "dataset should be one of" + " " + str(available_sets)
    )
    ...
    if dataset_name == "sa":
        image = io.loadmat(
            os.path.join(dataset_dir, dataset_name, "Salinas_corrected.mat")
        )
        ...
        image = io.loadmat(
            os.path.join(dataset_dir, dataset_name, "WHU_Hi_LongKou.mat")
        )
        image = image["WHU_Hi_LongKou"]
        gt = io.loadmat(
            os.path.join(dataset_dir, dataset_name, "WHU_Hi_LongKou_gt.mat")
        )
        ...
        image = io.loadmat(
            os.path.join(dataset_dir, dataset_name, "PHI_FangluTeaFarm.mat")
        )
        image = image["PHI_FangluTeaFarm"]
        gt = io.loadmat(
            os.path.join(dataset_dir, dataset_name, "PHI_GroundTruthFanglu.mat")
        )
    ...
    if dataset_name == "ip":
        image = io.loadmat(
            os.path.join(dataset_dir, dataset_name, "Indian_pines_corrected.mat")
        )
    ...
    if dataset_name == "ipnpy":
        # Indian Pines loaded from .npy (200 bands, full scene)
        image = np.load(
            os.path.join(dataset_dir, "Indian Pines", "indianpinearray.npy")
        )
        gt = np.load(os.path.join(dataset_dir, "Indian Pines", "IPgt.npy"))
        labels = [
            "Undefined",
            "Alfalfa",
            "Corn-notill",
    ...
    if np.count_nonzero(nan_mask) > 0:
        print(
            "warning: nan values found in dataset {}, using 0 replace them".format(
                dataset_name
            )
        )
        image[nan_mask] = 0
...
class HSIDataset(torch.utils.data.Dataset):
    def __init__(self, image, gt, patch_size, data_aug=True, return_dict=False):
        """
        ...
        self.data = np.pad(
            image, ((self.ps, self.ps), (self.ps, self.ps), (0, 0)), mode="reflect"
        )
        self.label = np.pad(
            gt, ((self.ps, self.ps), (self.ps, self.ps)), mode="reflect"
        )
    ...
    def __getitem__(self, i):
        ...
        if self.return_dict:
            return {
                "inputs": data,
                "targets": label,
                "indices": torch.tensor([x - self.ps, y - self.ps], dtype=torch.long),
            }
    ...
    def data_collator(batch):
        ...
        if isinstance(valid_batch[0], dict):
            output = {
                "inputs": torch.stack([item["inputs"] for item in valid_batch]),
                "targets": torch.stack([item["targets"] for item in valid_batch]),
            }
            if "indices" in valid_batch[0]:
                output["indices"] = torch.stack(
                    [item["indices"] for item in valid_batch]
                )
            return output
