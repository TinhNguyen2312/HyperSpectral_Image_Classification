...
def Draw(model,image,gt,patch_size,dataset_name,model_name,num_classes):
    height = gt.shape[0]
    width = gt.shape[1]
        ...
        for j in range(width):
            if int(gt[i, j]) == -1:
                ...
                X_test_image = torch.FloatTensor(
                    image_patch.transpose(0, 4, 3, 1, 2)).to(
                    device)
...
def setup_distributed():
    ...
    if world_size > 1:
        ...
        print(
            f"Setting up distributed training: rank={rank}, local_rank={local_rank}, "
...
def check_state_dict(model, state_dict: dict) -> bool:
    ...
    if not ckpt_keys or not ckpt_keys.issubset(model_keys):
        return False

    return all(model_state[k].shape == state_dict[k].shape for k in ckpt_keys)